...
class Unsigned(cantools.database.can.Signal):
	def __init__(self, name, start, length, unit='', scale=1, range=[0, 0]):
		super().__init__(name, start, length, unit=unit, scale=scale, minimum=range[0], maximum=range[1])

class Signed(cantools.database.can.Signal):
	def __init__(self, name, start, length, unit='', scale=1, range=[0, 0]):
		super().__init__(name, start, length, unit=unit, scale=scale, minimum=range[0], maximum=range[1], is_signed=True)
...
class Float(cantools.database.can.Signal):
	def __init__(self, name, start, unit='', scale=1, is_double=False):
		super().__init__(name, start, 32, unit=unit, scale=scale, is_float=True)
...
class Enum(cantools.database.can.Signal):
	def __init__(self, name, start, length, list):
		...
		super().__init__(
			...
			start, 
			length, 
			choices=collections.OrderedDict(list), 
			minimum=minimum,
			maximum=maximum
		)
